...
#run smartpca
bin/smartpca -p data/${SPECIES}/$PREFIX.par
```   
...
read_pca <- function(species, sample_file, prefix, nPCs = 20) {
  ...
  PCA <- read_delim(paste0(species_path, ".eigs"), 
                    skip=1, trim_ws = TRUE, delim=" ", 
                    col_names = c("ID", paste0("PC",(1:nPCs)), "CaseControl")) %>% 
    select(-CaseControl) %>% separate(ID, into=c("ID"), extra="drop") %>%
    full_join(samples, by=c("ID" = "sample")) %>%
    mutate(abr = abbreviate(pop, minlength=3)) %>% select(pop, ID, PC1:PC20, abr)
  eig.val <- sqrt(unlist(read.table(paste0(species_path,".eval")))[1:nPCs])
  sum.eig = sum(unlist(read.table(paste0(species_path,".eval"))))
  
  # Read in snp weightings matrix
  snpeigs = read.table(paste0(species_path,".snpeigs"))
...
screeplot(pmajPC,type='pve')  + ylim(0,0.15) +
